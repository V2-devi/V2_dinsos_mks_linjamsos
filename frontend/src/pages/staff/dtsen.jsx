...
const uploadSuratKematianToDB = async (no_kk, anggotaId, file) => {
  ...
  const res = await fetch(
    `https://backend-fastapi-ruby.vercel.app/keluarga/${no_kk}/anggota/${anggotaId}/upload-surat-kematian`,
    {
      method: "POST",
      headers: { Authorization: `Bearer ${token}` },
      body: formData
...
const handleSubmitFotoPPKS = async (ppksId = null) => {
  ...
  try {
    ...
    const res = await fetch("https://backend-fastapi-ruby.vercel.app/ppks/upload/foto-ppks", {
      method: "POST",
      headers: { Authorization: `Bearer ${token}` },
      body: formData
    });
...
const fetchAnggota = async (no_kk) => {
  try {
    const token = localStorage.getItem("token");
    const response = await fetch(`https://backend-fastapi-ruby.vercel.app/keluarga/${no_kk}/anggota`, {
      method: "GET",
      headers: { Authorization: `Bearer ${token}` }
    });
...
const handleAddAnggotaSubmit = async (e) => {
  ...
  try {
    ...
    // =====================================
    const response = await fetch(`https://backend-fastapi-ruby.vercel.app/keluarga/${selectedDtsenData.no_kk}/anggota`, {
      method: "POST",
      ...
      if (formAnggota.surat_kematian instanceof File) {
        try {
          ...
          const uploadRes = await fetch(
            `https://backend-fastapi-ruby.vercel.app/keluarga/${selectedDtsenData.no_kk}/anggota/${newAnggotaId}/upload-surat-kematian`,
            {
              method: "POST",
              headers: { Authorization: `Bearer ${token}` },
              body: formDataPDF
...
const handleEditAnggotaSubmit = async (e) => {
  ...
  try {
    ...
    const endpoint = `https://backend-fastapi-ruby.vercel.app/keluarga/${selectedDtsenData.no_kk}/anggota/${selectedAnggotaData.id}`;
    ...
    if (newSuratKematianFile instanceof File) {
      try {
        ...
        const uploadRes = await fetch(
          `https://backend-fastapi-ruby.vercel.app/keluarga/${selectedDtsenData.no_kk}/anggota/${selectedAnggotaData.id}/upload-surat-kematian`,
          {
            method: "POST",
            headers: { Authorization: `Bearer ${token}` },
            body: formDataPDF
  ...
  const handleEditAsetSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`https://backend-fastapi-ruby.vercel.app/aset/${no_kk}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`
 ...
 const handleAddPPKSSubmit = async (e) => {
  ...
  try {
    ...
    // =====================================
    const res = await fetch("https://backend-fastapi-ruby.vercel.app/ppks/", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`
    ...
    try {
      ...
      const uploadRes = await fetch("https://backend-fastapi-ruby.vercel.app/ppks/upload/foto-ppks", {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
        body: formDataFoto
      });
...
const handleUpdateStatusPPKS = async (e, statusBaru) => {
  ...
  try {
    ...
    const res = await fetch(`https://backend-fastapi-ruby.vercel.app/ppks/${selectedPPKSData.id}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${token}`
            ...
            <div style={{ marginTop: '25px', display: 'flex', justifyContent: 'flex-end', borderTop: '1px solid #e2e8f0', paddingTop: '20px' }}>
              <button
                ...
                onClick={async () => {
                  ...
                  const token = localStorage.getItem("token");
                  await fetch(`https://backend-fastapi-ruby.vercel.app/ppks/${selectedPPKSData.id}`, {
                    method: "PUT",
                    headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
